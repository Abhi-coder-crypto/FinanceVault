...
export default function DocumentCard({
  ...
  return (
    <Card className="hover-elevate transition-all duration-300 group relative overflow-hidden border border-slate-700/50 hover:border-emerald-500/50 shadow-lg" data-testid={`card-document-${fileName}`}>
      ...
      <CardHeader className="pb-3 relative space-y-0">
        <div className="flex items-center justify-between gap-3 mb-3">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-emerald-600/20 to-teal-600/20 flex items-center justify-center flex-shrink-0 ring-1 ring-emerald-500/20 group-hover:ring-emerald-400/40 transition-all duration-300">
              <FileText className="h-6 w-6 text-emerald-400" />
            </div>
            <div className="flex-1 min-w-0">
              <Tooltip>
                <TooltipTrigger asChild>
                  <h3 className="font-semibold text-base leading-tight truncate text-slate-100" data-testid={`text-filename-${fileName}`}>
                    {fileName}
                  </h3>
                </TooltipTrigger>
              ...
              {isAdmin && clientPhoneNumber && (
                <p className="text-xs text-slate-400 font-mono mt-1" data-testid={`text-client-${clientPhoneNumber}`}>
                  {clientPhoneNumber}
                </p>
              )}
            </div>
          </div>
          <Badge variant="secondary" className="text-xs font-semibold bg-emerald-600/20 text-emerald-400 border border-emerald-600/30 flex-shrink-0">
            {fileExtension}
          </Badge>
        </div>
      </CardHeader>
      ...
      <CardFooter className="pt-3 border-t border-slate-700/50 flex gap-2 flex-wrap relative">
        {onPreview && (
          <Button
            variant="outline"
            size="sm"
            ...
            className="flex-1 border-slate-600 text-slate-200 hover:text-emerald-400 hover:border-emerald-500/50"
          >
